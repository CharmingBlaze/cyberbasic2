...
- **Hybrid loop:** You define **update(dt)** and/or **draw()** (Sub or Function) and use a game loop with an **empty body** (`WHILE NOT WindowShouldClose() WEND`). The compiler now routes that loop through the shared runtime frame step: GetFrameTime, accumulate time, run zero or more fixed physics steps at `FixedDeltaTime()`, call `OnFixedUpdate(label$)` if registered, then run `update(dt)`, `draw()`, and render-queue flush. Use the hybrid loop when you want a clear update/draw split and automatic physics stepping. See [Program Structure](PROGRAM_STRUCTURE.md#hybrid-updatedraw-loop) and [Rendering and the game loop](RENDERING_AND_GAME_LOOP.md).
...
Make sure you are using the **game loop** (`WHILE NOT WindowShouldClose() WEND` or `REPEAT ... UNTIL WindowShouldClose()`) and that you have defined **draw()** (usually alongside `update(dt)`). The hybrid pipeline only runs when the compiler detects that loop pattern together with `update(dt)` and/or `draw()`. See [Program Structure – hybrid loop](PROGRAM_STRUCTURE.md#hybrid-updatedraw-loop).
