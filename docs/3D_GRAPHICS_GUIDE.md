...
Every 3D program needs a window, a 3D camera, and a loop with your draw calls. The compiler does not inject any frame or mode calls (DBPro-style). When you use the **hybrid loop** (define **update(dt)** and **draw()**), 3D draw calls in **draw()** are queued and flushed in order (2D then 3D then GUI). **When using UseUnifiedRenderer:** call **SYNC** instead of Start3D/End3D; the engine handles 3D→2D→GUI order. See [Program Structure](PROGRAM_STRUCTURE.md#hybrid-updatedraw-loop) and [Rendering and the game loop](RENDERING_AND_GAME_LOOP.md).
...
- **Physics:** `CreateWorld3D`, `CreateSphere3D`/`CreateBox3D`, `Step3D`, `GetPositionX3D`/`GetPositionY3D`/`GetPositionZ3D`, and **GAME.MoveWASD**, **GAME.CameraOrbit** for character and camera. See [Game Development Guide](GAME_DEVELOPMENT_GUIDE.md) and [templates/3d_game.bas](../templates/3d_game.bas).
